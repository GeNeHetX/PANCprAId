...
class MILModel(nn.Module):
    ...
    def forward(self, bags, lens):
        ...
        #if bags.dtype != torch.float32:
        #    bags = torch.tensor(bags,dtype=torch.float32)
...
def loadPANCprAId(gem_pth,ffx_pth,input_dim,device) :
    hFFXmodel = load_model(ffx_pth,input_dim,device)
    hGEMmodel = load_model(gem_pth,input_dim,device)

    return hFFXmodel, hGEMmodel
